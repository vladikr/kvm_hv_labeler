...
import (
	...
	//gomsr "github.com/fearful-symmetry/gomsr"
)
...
func (s *CapScan) populateCaps() {
	...
	s.supprtedMSRS = make(map[uint32]bool)
	for _, kvmMsr := range msrs {
		s.supprtedMSRS[kvmMsr] = true
	}
}
...
func (s *CapScan) exposeCapabilities(exposeEnabled bool) []string {
	...
	for _, capb := range CapsDesc {
		shouldExpose := true
		if capb.MSR != 0 {
			_, isEnabled := s.supprtedMSRS[uint32(capb.MSR)]
...
func (s *CapScan) getLabels(exposeEnabled bool) []string {
	...
	s.populateCaps()
	caps := s.exposeCapabilities(exposeEnabled)
	for idx, capb := range caps {
		caps[idx] = fmt.Sprintf("%s%s%s", prefix, capb, suffixLabel)
	}
	return caps
}
...
func main() {
	...
	fmt.Println("Enabled: ", capScanner.getLabels(true))
	fmt.Println("Missing: ", capScanner.getLabels(false))
}
